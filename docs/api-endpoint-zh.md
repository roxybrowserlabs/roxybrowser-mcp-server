...
| ---------------------------- | ------------------ | -------------------------- |
| code                         | int                | 状态码，0：成功，500：失败 |
| data                         | object             | API 返回的全部空间。       |
| data.total                   | int                | 总数                       |
| data.rows                    | List&lt;object&gt; | 空间摘要列表               |
| data.rows[].email            | string             | 账号邮箱                   |
| data.rows[].id               | int \| string      | 空间 ID                    |
| data.rows[].maxWindowCount   | int                | 最大窗口数                 |
| data.rows[].role             | int                | 空间角色                   |
| data.rows[].totalMemberCount | int                | 成员总数                   |
| data.rows[].totalWindowCount | int                | 窗口总数                   |
| data.rows[].useMemberCount   | int                | 已用成员数                 |
| data.rows[].useWindowCount   | int                | 已用窗口数                 |
| data.rows[].workspaceName    | string             | 空间名称                   |
| data.rows[].workspaceNo      | string             | 空间编号                   |
| msg                          | string             | 返回结果                   |
...
| 参数名称    | 必需项 | 参数类型      | 默认值 | 描述                             |
| ----------- | ------ | ------------- | ------ | -------------------------------- |
| workspaceId | 是     | int \| string | -      | 目标工作区 ID                    |
| force       | 否     | boolean       | false  | 强制切换，即使仍有浏览器窗口打开 |

#### force 规则

`force` 为 `false` 或省略时，如果有浏览器窗口打开则拒绝切换；`force` 为 `true` 时忽略浏览器窗口状态并直接切换。
...
| ---------------------------------- | ------------------ | -------------------------- |
| code                               | int                | 状态码，0：成功，500：失败 |
| data                               | object             | API 返回的团队工作区。     |
| data.id                            | int \| string      | 团队ID                     |
| data.workspaceName                 | string             | 团队名称                   |
