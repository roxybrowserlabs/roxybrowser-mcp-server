...
import { formatPlatformAccounts, formatProfile, formatProfiles, normalizeProxyListArgs } from './formatters.js'
...
export const BROWSER_MCP_TOOLS: McpTool[] = [
  {
    ...
    handler: async (args, context) => {
      ...
      return page.rows.length === 0
        ? 'No workspaces found.'
        : [`Found ${page.total} workspace(s).`, ...page.rows.map(workspace => `- ${workspace.id}: ${workspace.name}`)].join('\n')
    },
  },
  {
    ...
    handler: async (args, context) => {
      ...
      return page.rows.length === 0
        ? 'No projects found.'
        : [`Found ${page.total} project(s).`, ...page.rows.map(project => `- ${project.id}: ${project.name}`)].join('\n')
    },
  },
  {
    ...
    handler: async (_args, context) => {
      ...
      return labels.length === 0
        ? 'No labels found.'
        : [`Found ${labels.length} label(s).`, ...labels.map(label => `- ${label.id}: ${label.name} ${label.color ?? ''}`.trimEnd())].join('\n')
    },
  },
  {
  ...
  },
  {
    ...
    handler: async (args, context) => {
      const opened = await context.browser!.profiles.open(args.id, removeUndefined({ force: args.force, args: args.args, headless: args.headless }))
  ...
  },
  {
    ...
    handler: async (args, context) => {
      await context.browser!.profiles.clearLocalCache(args.ids, removeUndefined({ type: args.type }))
  ...
  },
  {
    ...
    handler: async (args, context) => {
      ...
      return page.rows.length === 0
        ? 'No proxies found.'
        : [`Found ${page.total} proxy/proxies.`, ...page.rows.map(proxy => `- ${proxy.id}: ${proxy.source} ${proxy.protocol ?? 'N/A'} ${proxy.host ?? 'N/A'}:${proxy.port ?? 'N/A'}`)].join('\n')
    },
  },
  {
    name: 'roxy_proxy_get',
    ...
    handler: async (args, context) => JSON.stringify(await context.browser!.proxies.get(args.id), null, 2),
  },
  {
  ...
  },
  {
    ...
    inputSchema: objectSchema({
      proxies: { type: 'array', items: objectSchema(proxyInputSchema, ['protocol', 'host', 'port']) },
    }, ['proxies']),
    handler: async (args, context) => {
  ...
  },
  {
    ...
    inputSchema: objectSchema({ id: { type: 'number' } }, ['id']),
    handler: async (args, context) => JSON.stringify(await context.browser!.proxies.detect(args.id), null, 2),
  },
  {
    ...
    inputSchema: objectSchema({}),
    handler: async (_args, context) => JSON.stringify(await context.browser!.proxies.detectChannels(), null, 2),
  },
  {
    ...
    inputSchema: objectSchema(paginationSchema),
    handler: async (args, context) => formatPlatformAccounts(await context.browser!.platformAccounts.list(args)),
  },
  {
  ...
  },
  {
    ...
    inputSchema: objectSchema({
      accounts: { type: 'array', items: objectSchema(platformAccountInputSchema, ['platformUrl']) },
    }, ['accounts']),
    handler: async (args, context) => {
