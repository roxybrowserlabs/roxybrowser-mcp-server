  ...
  return {
    ...
    profiles: {
      list: async params => {
        ...
        return { page: params.page ?? 1, pageSize: params.pageSize ?? 20, total: 1, rows: [profile] }
      },
      get: async id => {
        calls.push(['get', id])
        return { ...profile, id }
...
describe('CommerceAccountDomain', () => {
  test('renames browser profile workflows into ecommerce account workflows', async () => {
    ...
    assert.equal(calls.find(call => call[0] === 'create')[1].platformAccounts[0].platformUrl, 'https://sellercentral.amazon.com')
    assert.deepEqual(calls.filter(call => call[0] === 'create')[1][1].urls, ['https://example.com/start'])
