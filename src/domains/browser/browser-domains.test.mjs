  ...
  const api = {
    ...
    browser: {
      ...
      open: async params => {
        ...
        return ok({ dirId: params.dirId, ws: `ws://${params.dirId}`, http: `http://${params.dirId}` })
      },
      close: async params => {
    ...
    proxy: {
      ...
      detail: async params => {
        ...
        return ok({ total: 1, rows: [{ id: params.id, dataType: 'proxyModule', host: '127.0.0.1' }] })
      },
      create: async params => {
    ...
    account: {
      list: async params => {
        ...
        return ok({
          total: 1,
          rows: [{ id: 5, platformUrl: 'https://example.com', platformUserName: 'seller', platformRemarks: 'memo' }],
        })
      },
      create: async params => {
...
describe('browser domains', () => {
  test('maps workspaces and projects into SDK names', async () => {
    ...
    assert.deepEqual(workspaces.rows[1].projects, [{ id: 4, name: 'Fallback Ops' }, { id: 0, name: '' }])
  ...
  test('maps profile operations to browser endpoints', async () => {
    ...
    const page = await profiles.list({ page: 1, pageSize: 20, projectIds: [3, 4], name: 'Alpha', serialNumber: '11', os: 'Windows' })
    ...
    const openedOne = await profiles.open('profile-1', { force: true, args: ['--mute-audio'], headless: true })
    ...
    assert.equal(calls.find(call => call[0] === 'browser.create')[1].windowPlatformList[0].platformUserName, 'seller')
    assert.equal(calls.find(call => call[0] === 'browser.delete')[1].isSoftDelete, false)
    assert.deepEqual(calls.find(call => call[0] === 'browser.connectionInfo')[1], { dirIds: 'profile-1' })
  })
  ...
  test('reports missing profiles and failed SDK responses clearly', async () => {
    ...
    assert.throws(() => ensureSuccess({ code: 500, msg: '' }), /Roxy API request failed with code 500/)
  ...
  test('maps proxy operations through list_merged and proxy source language', async () => {
    ...
    await proxies.create({ protocol: 'SOCKS5', host: '127.0.0.1', port: '1080', username: 'u', password: 'p', remark: 'memo' })
    ...
    await proxies.list({ source: 'user', type: 'available', bindStatus: 'bound', autoRenew: true, checkStatus: 'passed' })
  ...
  test('maps platform account CRUD operations', async () => {
    ...
    const createdId = await accounts.create({ platformUrl: 'https://example.com', username: 'seller', password: 'secret', twoFactorKey: 'otp', remarks: 'memo' })
    ...
    assert.equal(calls.find(call => call[0] === 'account.batchCreate')[1].accountList[0].platformUserName, 'seller')
  })
})
