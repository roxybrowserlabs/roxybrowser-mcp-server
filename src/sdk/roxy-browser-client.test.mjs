    ...
    fetch: async (url, options) => {
      ...
      if (url.includes('/browser/detail')) {
        return createJsonResponse({
          ...
          data: {
            total: 1,
            rows: [{ dirId: 'profile-1', windowName: 'Alpha', coreType: 'Chrome', coreVersion: '140', os: 'Windows', osVersion: '11' }],
          },
        })
      }
...
describe('RoxyBrowserClient', () => {
  test('maps profile list params to backend browser list params', async () => {
    ...
    const page = await client.profiles.list({ page: 2, pageSize: 10, projectIds: [1, 2], name: 'Alpha' })
  ...
  test('maps proxy filters to merged proxy list params', async () => {
    ...
    try {
      ...
      await client.proxies.list({ source: 'user', type: 'available', bindStatus: 'bound', autoRenew: true, sortBy: 'lastCountry', sortOrder: 'asc' })
  ...
  test('exposes labels and propagates label errors', async () => {
    ...
    const success = new RoxyBrowserClient({
      ...
      fetch: async (url, options) => {
        ...
        return createJsonResponse({ code: 0, msg: 'ok', data: [{ id: 1, name: 'VIP', color: '#ff0000' }] })
      },
    })
