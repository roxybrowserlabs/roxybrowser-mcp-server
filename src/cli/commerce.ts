...
export async function runCommerceCli(argv = process.argv): Promise<void> {
  ...
  program
    ...
    .option('-w, --workspace-id <id>', 'Default workspace ID', value => Number.parseInt(value, 10))
    .option('-t, --timeout <ms>', 'Request timeout in milliseconds', value => Number.parseInt(value, 10), process.env.ROXY_TIMEOUT ? Number(process.env.ROXY_TIMEOUT) : 30_000)
