...
function printUsage() {
  console.log(`Usage:
  npm run skills:apply [-- --manifest skills/skills.json]
  npm run skills:check [-- --manifest skills/skills.json]
  npm run skills:pull -- <url> [--manifest skills/skills.json] [--no-apply]
...
function normalizeSkill(skill, index) {
  ...
  const path = skill.path === undefined
...
async function applyManifest(manifestPath, { checkOnly = false } = {}) {
  ...
  console.log(`Skill metadata ${verb}. ${changed.length} file(s) ${checkOnly ? 'would change' : 'updated'}.`)
}
...
async function main() {
  ...
  if (command === 'pull') {
    ...
    if (!url)
      throw new Error('pull requires a metadata JSON URL')
