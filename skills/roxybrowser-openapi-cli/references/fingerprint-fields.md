...
Both `roxy_profile_create` and `roxy_profile_update` accept `fingerInfo`. For create, place it inside the relevant item in `profiles`; for update, place it beside `dirId`. Use the field names shown here.
