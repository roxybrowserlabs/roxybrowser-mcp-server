# Fingerprint Timezone Values

Use this file for exact `timeZone` values used by `fingerInfo`. Other appendix values live in [fingerprint-resolutions.md](fingerprint-resolutions.md), [fingerprint-languages.md](fingerprint-languages.md), and [fingerprint-interface-languages.md](fingerprint-interface-languages.md). Field meanings live in [fingerprint-fields.md](fingerprint-fields.md).

Source: user-provided RoxyBrowser API appendix attachment. Count from this file: 478 timezones.

`timeZone` must use the full appendix string, including the GMT prefix and zone name. Do not pass only the IANA suffix, a city name, or a numeric offset.
